let canvas;
let ctx;
let startGrad1, endGrad1, startGrad2, endGrad2;
let slider1;
let slider2;
let sizeRect;
let scroll;
...
function draw() {
	...
	push();
	noStroke();
	translate(width * 0.75, map(scroll, 0, height, 0, 42));
	push();
	fill(217);
	ellipse(3, -501, 84);
	pop();
	...
	for (let star of stars) {
		star.run(map(scroll, 0, height, 0, 42));
	}
	...
	translate(width*0.5,map(scroll, 0, height, 0, 42));
